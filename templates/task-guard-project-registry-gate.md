...
routing_source：/Users/hula/workspace/multiagent-orchestration-system/config/projects.yaml  
...
并使用 registry 做项目边界检查，防止 ASK、multiagent-orchestration-system、父级 `/Users/hula/workspace` 被混淆。

本文件是 multiagent-orchestration-system 仓库内的项目治理模板，不是 Hermes core 修改，不直接写 Hermes 内部 DB。

## 2. 启动前强制读取

- task_guard 启动或接收任务前必须读取 `config/projects.yaml`。
- registry 缺失、YAML 解析失败、缺少 `projects.ask` 或 `projects.multiagent-orchestration-system` 时，不执行任务。
- 读取失败时必须输出阻断项目头，并设置：
  - `project_id=blocked`
...
| multiagent-orchestration-system | `/Users/hula/workspace/multiagent-orchestration-system` |
...
## 7. multiagent-orchestration-system gate

对 multiagent-orchestration-system：
...
- [ ] 已读取 `/Users/hula/workspace/multiagent-orchestration-system/config/projects.yaml`。
- [ ] registry YAML 可解析。
- [ ] projects 包含 `ask` 和 `multiagent-orchestration-system`。
