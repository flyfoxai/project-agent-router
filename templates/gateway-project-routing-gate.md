...
如果 registry 缺失、YAML 不可解析、缺少 `ask` 或 `multiagent-orchestration-system`，必须阻断：
...
project_display_name: "ASK | Multi-Agent Orchestration System | blocked"
...
## 7. multiagent-orchestration-system 规则

multiagent-orchestration-system 当前 registry 事实：
...
- [ ] registry 包含 `ask` 与 `multiagent-orchestration-system`。
- [ ] project_id 解析优先级已固化。
- [ ] 只靠 cwd 无法唯一映射时阻断。
